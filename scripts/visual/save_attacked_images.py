...
# create a pgd adversarial image
...
# convert normalised image back to uint8 image
...
def main():
    ...
    for i in range(N_IMAGES):
        ...
        # save clean and fgsm images
        clean_img.save(clean_path)
        fgsm_img.save(fgsm_path)

        pgd_path = None

        # save pgd image if enabled
        if ALSO_SAVE_PGD:
        ...
        print(
            f"[{i:03d}] true={classes[y]:10s} | "
            f"clean={classes[clean_pred.item()]:10s} ({clean_conf.item()*100:5.1f}%) | "
            f"fgsm={classes[fgsm_pred.item()]:10s} ({fgsm_conf.item()*100:5.1f}%)"
            + (
                f" | pgd={classes[pgd_pred.item()]:10s} ({pgd_conf.item()*100:5.1f}%)"
                if ALSO_SAVE_PGD else ""
            )
        )
