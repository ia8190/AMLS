...
def fgsm_attack_norm(model, x_norm, y, eps_px, mean, std):
    """
    eps is specified in pixel space (0..1), applied in normalized space by scaling per-channel using std
    """
    model.eval()
...
def pgd_attack_norm(model, x_norm, y, eps_px, alpha_px, steps, mean, std, random_start=True):
    """
    PGD-Linf with eps/alpha in pixel-space (0..1), applied in normalized space by scaling per channel using std
    """
    model.eval()
    device = x_norm.device
    _, std_t, x_min, x_max = _norm_bounds(mean, std, device)
...
def eval_mode(
    ...
    Returns:
      acc (float)
      cm (tensor CxC)
      confidences (list of float, max softmax)
      attack_success_rate (float)  # fraction where clean was correct but attacked became wrong (if attack is not None)
    """
    y_all = []
    pred_all = []
    conf_all = []
    ...
    for x, y in loader:
        ...
        if attack is None:
            ...
            # Compute clean correctness for success-rate bookkeeping
            with torch.no_grad():
                pred_clean = model(x).argmax(1)
                clean_correct = (pred_clean == y)
                clean_correct_total += clean_correct.sum().item()

            # Create adversarial examples (grad needed inside attack)
            x_adv = attack(x, y)

            # Predict on adversarial
            pred, conf = batch_predict(model, x_adv)

            # success rate = clean-correct AND adversarial-wrong
            with torch.no_grad():
                adv_wrong = (pred != y)
...
def main():
    ...
    eps_x = [int(round(e * 255)) for e in eps_list]  # display as /255

    # PGD eval config (keep standard)
    pgd_alpha = 2/255
    pgd_steps = 7
    ...
    summary = {
        ...
        "settings": {
            ...
            "pgd": {"alpha": float(pgd_alpha), "steps": int(pgd_steps), "random_start": bool(pgd_random_start)},
        },
        "results": {}
    }

    # 1. Clean eval (acc + confusion + confidence) 
    print("\n=== CLEAN EVALUATION ===")
    b_clean_acc, b_clean_cm, b_clean_confs, _ = eval_mode(baseline, test_loader, device, "clean", attack=None)
    d_clean_acc, d_clean_cm, d_clean_confs, _ = eval_mode(defence,  test_loader, device, "clean", attack=None)

    print(f"Baseline clean acc: {b_clean_acc*100:.2f}%")
    print(f"Defence  clean acc: {d_clean_acc*100:.2f}%")

    plot_confusion_matrix(b_clean_cm, "Baseline - Confusion Matrix (Clean)", out_dir / "cm_baseline_clean.png")
    plot_confusion_matrix(d_clean_cm, "Defence - Confusion Matrix (Clean)", out_dir / "cm_defence_clean.png")

    b_clean_pc = per_class_accuracy(b_clean_cm)
    d_clean_pc = per_class_accuracy(d_clean_cm)
    plot_bars(
        [v*100 for v in b_clean_pc], [v*100 for v in d_clean_pc],
        "Baseline", "Defence",
    ...
    # 2. Robustness curves for FGSM + PGD (acc + ASR) 
    ...
    # also keep conf distributions at eps=8/255 for confidence hist
    ...
    for eps in eps_list:
        if eps == 0:
            # clean
            b_fgsm_accs.append(b_clean_acc); d_fgsm_accs.append(d_clean_acc)
            b_pgd_accs.append(b_clean_acc);  d_pgd_accs.append(d_clean_acc)
            b_fgsm_asr.append(0.0); d_fgsm_asr.append(0.0)
            b_pgd_asr.append(0.0);  d_pgd_asr.append(0.0)
            print(f"eps=0/255 | FGSM (B/D) {b_clean_acc*100:.1f}% / {d_clean_acc*100:.1f}% | PGD (B/D) {b_clean_acc*100:.1f}% / {d_clean_acc*100:.1f}%")
            continue

        # --- FGSM ---
        def fgsm_attack_for(model):
            return lambda x, y: fgsm_attack_norm(model, x, y, eps_px=eps, mean=mean, std=std)

        b_fgsm_acc, _, _, b_asr = eval_mode(baseline, test_loader, device, "fgsm", attack=fgsm_attack_for(baseline))
        d_fgsm_acc, _, _, d_asr = eval_mode(defence,  test_loader, device, "fgsm", attack=fgsm_attack_for(defence))

        b_fgsm_accs.append(b_fgsm_acc); d_fgsm_accs.append(d_fgsm_acc)
        b_fgsm_asr.append(b_asr); d_fgsm_asr.append(d_asr)

        # --- PGD ---
        def pgd_attack_for(model):
            return lambda x, y: pgd_attack_norm(
                model, x, y,
                eps_px=eps, alpha_px=pgd_alpha, steps=pgd_steps,
                mean=mean, std=std, random_start=pgd_random_start
            )

        b_pgd_acc, b_cm, b_confs, b_asr_pgd = eval_mode(baseline, test_loader, device, "pgd", attack=pgd_attack_for(baseline))
        d_pgd_acc, d_cm, d_confs, d_asr_pgd = eval_mode(defence,  test_loader, device, "pgd", attack=pgd_attack_for(defence))

        b_pgd_accs.append(b_pgd_acc); d_pgd_accs.append(d_pgd_acc)
        b_pgd_asr.append(b_asr_pgd); d_pgd_asr.append(d_asr_pgd)

        # store eps=8/255 assets for deeper analysis
        ...
        print(f"eps={int(round(eps*255))}/255 | FGSM acc (B/D): {b_fgsm_acc*100:5.1f}% / {d_fgsm_acc*100:5.1f}% | PGD acc (B/D): {b_pgd_acc*100:5.1f}% / {d_pgd_acc*100:5.1f}%")

    # Curves
    plot_curve(
        eps_x, [v*100 for v in b_fgsm_accs], [v*100 for v in d_fgsm_accs],
        "Baseline", "Defence",
    ...
    plot_curve(
        eps_x, [v*100 for v in b_pgd_accs], [v*100 for v in d_pgd_accs],
        "Baseline", "Defence",
    ...
    plot_curve(
        eps_x, [v*100 for v in b_fgsm_asr], [v*100 for v in d_fgsm_asr],
        "Baseline", "Defence",
    ...
    plot_curve(
        eps_x, [v*100 for v in b_pgd_asr], [v*100 for v in d_pgd_asr],
        "Baseline", "Defence",
    ...
    # 3. Confusion + per-class at PGD eps=8/255 
    if b_pgd_cm_eps8 is not None and d_pgd_cm_eps8 is not None:
        plot_confusion_matrix(b_pgd_cm_eps8, "Baseline - Confusion Matrix (PGD eps=8/255)", out_dir / "cm_baseline_pgd_eps8.png")
        plot_confusion_matrix(d_pgd_cm_eps8, "Defence - Confusion Matrix (PGD eps=8/255)", out_dir / "cm_defence_pgd_eps8.png")

        b_pgd_pc = per_class_accuracy(b_pgd_cm_eps8)
        d_pgd_pc = per_class_accuracy(d_pgd_cm_eps8)

        plot_bars(
            [v*100 for v in b_pgd_pc], [v*100 for v in d_pgd_pc],
            "Baseline", "Defence",
    ...
    # 4. Confidence histograms (clean vs PGD eps=8/255) 
    if b_pgd_confs_eps8 is not None and d_pgd_confs_eps8 is not None:
        plot_hist(
            b_clean_confs, b_pgd_confs_eps8,
            "Baseline Clean", "Baseline PGD eps=8/255",
            "Baseline Confidence Distribution (Clean vs PGD eps=8/255)",
            out_dir / "hist_conf_baseline_clean_vs_pgd8.png"
        )
        plot_hist(
            d_clean_confs, d_pgd_confs_eps8,
            "Defence Clean", "Defence PGD eps=8/255",
    ...
    summary["results"] = {
        "clean_accuracy": {"baseline": b_clean_acc, "defence": d_clean_acc},
        "fgsm_accuracy_curve": {"eps_255": eps_x, "baseline": b_fgsm_accs, "defence": d_fgsm_accs},
        "pgd_accuracy_curve":  {"eps_255": eps_x, "baseline": b_pgd_accs,  "defence": d_pgd_accs},
        "fgsm_asr_curve": {"eps_255": eps_x, "baseline": b_fgsm_asr, "defence": d_fgsm_asr},
        "pgd_asr_curve":  {"eps_255": eps_x, "baseline": b_pgd_asr,  "defence": d_pgd_asr},
        "per_class_clean": {"baseline": b_clean_pc, "defence": d_clean_pc},
    }
    if b_pgd_cm_eps8 is not None and d_pgd_cm_eps8 is not None:
        summary["results"]["per_class_pgd_eps8"] = {"baseline": b_pgd_pc, "defence": d_pgd_pc}
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["Model", "CleanAcc(%)", "PGD@8/255 Acc(%)", "FGSM@8/255 Acc(%)"])
        # eps=8/255 index:
        ...
        w.writerow(["Baseline", f"{b_clean_acc*100:.2f}", f"{(b_pgd8*100 if b_pgd8 is not None else ''):.2f}", f"{(b_fgsm8*100 if b_fgsm8 is not None else ''):.2f}"])
        w.writerow(["Defence",  f"{d_clean_acc*100:.2f}", f"{(d_pgd8*100 if d_pgd8 is not None else ''):.2f}", f"{(d_fgsm8*100 if d_fgsm8 is not None else ''):.2f}"])
